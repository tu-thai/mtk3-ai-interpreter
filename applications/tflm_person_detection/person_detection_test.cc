...
#include "person_detection_test.h"
...
TfLiteStatus LoadPersonDetectionModelAndPerformInference() {
  ...
  TFLITE_CHECK_EQ(kTfLiteOk, invoke_status);
  ...
  TFLITE_CHECK_EQ(kCategoryCount, output->dims->data[1]);
...
int person_detection_test(void) {
  tflite::InitializeTarget();
  LoadPersonDetectionModelAndPerformInference();
